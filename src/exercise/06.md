...
- Forms come with inbuilt events like onSubmit etc., which allow us to define
  different user interactions.
- `event.target.elements` has the array of input passed to the forms, something
  we can figure out by `console.dir(event.target.elements)` instead of
  `console.log` as some elements of DOM are not fully rendered with
  `console.log`.
- React has concepts like `useRef`, `useState` that lets you either refer to an
  input via the current value or lets you track state and updater for a counter
  like object.
- We can control input value by explicitly setting `value` field within the
  input element of the form. This will result in a read-only behavior unless we
  also accomany it with an `onChange` handler.
