...
- Learnt about JSX Functions for returning HTML elements to be returned.
- Parameter to a JSX function should be specified with braces as it needs to
  pick up the associated Javascript variable and not think of it as HTML
  formatting. Or we can pass props and then access props.parameter.
  - Example: function message({parameter})
- We can create custom type like `<div />` by creating a JSX function that
  starts with a capital letter and calling that: checkout `<Message />` example.
- We can use `propTypes` to define the type of the custom components and then do
  proper error handling when they're not passed as expected.
- `<> </>` allows to avoid nesting into `<div />` elements.
- It is awesome that `propTypes` is a development only feature, meaning babel
  will auto-remove such validation code from running on production mode.
- If we are already using typed languages like Typescript, we wouldn't need to
  define `PropTypes`, this is more for Javascript kind of languages.
